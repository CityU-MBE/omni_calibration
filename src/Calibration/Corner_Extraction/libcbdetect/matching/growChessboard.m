...
switch border_type
  ...
  case 1
    [pred,last] = predictCorners(p(chessboard(:,end-2),:),p(chessboard(:,end-1),:),p(chessboard(:,end),:));
    idx = assignClosestCorners(cand,pred,last);
    if idx~=0
  ...
  case 3
    [pred,last] = predictCorners(p(chessboard(:,3),:),p(chessboard(:,2),:),p(chessboard(:,1),:));
    idx = assignClosestCorners(cand,pred,last);
    if idx~=0
...
function [pred,p3] = predictCorners(p1,p2,p3)
...
function idx = assignClosestCorners(cand,pred,last)
...
for i=1:size(pred,1)
  w = pred(i,:)-last(i,:);
  v = cand-ones(size(cand,1),1)*last(i,:);
  v = v*[w;w(2) -w(1)]./(norm(w).*norm(w));
  d1 = atan2(v(:,2),v(:,1));
  d2 = (1-sqrt(v(:,1).*v(:,1)+v(:,2).*v(:,2)));
  D(:,i) = sqrt(abs(d1)+d2.*d2.*d2.*d2);
  % l2 norm:
