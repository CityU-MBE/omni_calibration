    ...
    for i = 1 : size(param.board,2)
        ...
        if (images{1,c}.active_images(i) == 1)
            ...
            if info.visualize_error_flag == 1
                l = dir( [fullfile( info.image_path, eval(sprintf('info.name_cam%1d',c))) '*.png']);
...
if info.visualize_error_flag == 1
    ...
    for n = 1 : size(param.board,2)
        if images{1,1}.active_images(n) == 1
            ...
            YYx=reshape(M_3d(1,:),5,7);
            YYy=reshape(M_3d(2,:),5,7);
            YYz=reshape(M_3d(3,:),5,7);
    ...
    plot3(h,-param.cam{2}.ex_T(1), -param.cam{2}.ex_T(2), -param.cam{2}.ex_T(3),'ko','MarkerFaceColor','k','MarkerSize',8);
